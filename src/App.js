...
class App extends React.Component {
    ...
    this.props.dispatch(getData()).then(() => {
      this.props.apiData.forEach((content, i) => {
        if (content.review_body + content.review_headline) {
          ...
          stateCopy.content[category][temp].push({
  ...
  handleClick(event) {
    ...
    if (category == "Positive") {
      this.setState({ PoisitivePage: Number(event.target.id) });
    } else if (category == "Negative") {
      this.setState({ NegativePage: Number(event.target.id) });
    } else if (category == "Neutral") {
      this.setState({ NeutralPage: Number(event.target.id) });
    }
  }
  ...
  renderCardtext(contentVal, category) {
    ...
    let indexOfLastItem;
    if (category == "Positive") {
      indexOfLastItem = this.state.PoisitivePage * 3;
    } else if (category == "Negative") {
      indexOfLastItem = this.state.NegativePage * 3;
    } else if (category == "Neutral") {
      indexOfLastItem = this.state.NeutralPage * 3;
    }
